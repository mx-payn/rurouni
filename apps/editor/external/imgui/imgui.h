...
IMGUI_API void ShowDemoWindow(
    bool* p_open =
        NULL);  // create Demo window. demonstrate most ImGui
...
IMGUI_API void SetWindowFocus(
    const char* name);  // set named window to be focused / top-most.
...
IMGUI_API void PushTabStop(
    bool tab_stop);  // == tab stop enable. Allow focusing using
...
IMGUI_API void Unindent(
    float indent_w =
        0.0f);  // move content position back to the left, by
                // indent_w, or style.IndentSpacing if indent_w <= 0
...
IMGUI_API void TextColored(const ImVec4& col, const char* fmt, ...) IM_FMTARGS(
    2);  // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt,
         // ...); PopStyleColor();
...
IMGUI_API bool SmallButton(
    const char* label);  // button with (FramePadding.y ==
...
IMGUI_API bool TableSetColumnIndex(
    int column_n);  // append into the specified column. Return
...
IMGUI_API ImDrawList*
GetBackgroundDrawList();  // get background draw list for the viewport
...
IMGUI_API ImDrawList*
GetForegroundDrawList();  // get foreground draw list for the viewport
                          // associated to the current window. this draw list
                          // will be the last rendered one. Useful to quickly
...
IMGUI_API const char* GetStyleColorName(
    ImGuiCol idx);  // get a string corresponding to the enum value
...
IMGUI_API bool IsMouseReleased(
    ImGuiMouseButton button);  // did mouse button released? (went
                               // from Down to !Down)
...
IMGUI_API int GetMouseClickedCount(
    ImGuiMouseButton
        button);  // return the number of successive mouse-clicks at
...
enum ImGuiWindowFlags_ {
    ...
    ImGuiWindowFlags_NavFlattened =
        1 << 23,  // [BETA] On child window: allow gamepad/keyboard navigation
...
// Obsolete names
    ...
    ImGuiWindowFlags_AlwaysUseWindowPadding =
        1
        << 30,  // Obsoleted in 1.90: Use ImGuiChildFlags_AlwaysUseWindowPadding
                // in BeginChild() call.
#endif
};
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_EscapeClearsAll =
        1
        << 20,  // Escape key clears content if not empty, and deactivate
...
enum ImGuiTreeNodeFlags_ {
    ...
    ImGuiTreeNodeFlags_SpanAvailWidth =
        1 << 11,  // Extend hit box to the right-most edge, even if not framed.
                  ...
                  // items on the same line. In the future we may refactor the
                  // hit system to be front-to-back, allowing natural overlaps
    ...
    ImGuiTreeNodeFlags_SpanAllColumns =
        1 << 13,  // Frame will span all columns of its container table (text
                  // will still fit in current column)
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
        1 << 14,  // (WIP) Nav: left direction may move to this TreeNode() from
...
enum ImGuiTabBarFlags_ {
    ...
    ImGuiTabBarFlags_NoCloseWithMiddleMouseButton =
        1 << 3,  // Disable behavior of closing tabs (that are submitted with
                 ...
                 // repro this behavior on user's side with if (IsItemHovered()
...
enum ImGuiTabItemFlags_ {
    ...
    ImGuiTabItemFlags_UnsavedDocument =
        1 << 0,  // Display a dot next to the title + tab is selected when
                 // clicking the X + closure is not assumed (will wait for user
                 // to stop submitting the tab). Otherwise closure is assumed
                 // when pressing the X, so if you keep submitting the tab may
...
enum ImGuiTableFlags_ {
    ...
    ImGuiTableFlags_PreciseWidths = 1
                                    << 19,  // Disable distributing remainder
                                            // width to stretched columns (width
    ...
    ImGuiTableFlags_NoPadInnerX = 1 << 23,  // Disable inner padding between
                                            // columns (double inner padding if
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_HasMouseHoveredViewport =
        1
        << 11,  // Backend Platform supports calling io.AddMouseViewportEvent()
...
struct ImGuiStyle {
    ...
    float TabMinWidthForCloseButton;  // Minimum width for close button to
                                      // appear on an unselected tab when
...
struct ImGuiIO {
    ...
    bool ConfigViewportsNoTaskBarIcon;  // = false          // Disable default
                                        ...
                                        // be set on it.
    bool
        ConfigViewportsNoDecoration;  // = true           // Disable default OS
    ...
    bool ConfigViewportsNoDefaultParent;  // = false          // Disable default
                                          ...
                                          // between the OS windows (some
                                          // backend may ignore this). Set to
                                          // true if you want the default to be
                                          // 0, then all viewports will be
                                          // top-level OS windows.
    ...
    bool ConfigDebugIgnoreFocusLoss;  // = false          // Ignore
                                      ...
                                      // not calling io.ClearInputKeys() in
                                      // input processing.
    ...
    IMGUI_API void
    ClearInputKeys();  // Clear current keyboard/mouse/gamepad state
    ...
    int KeyMap[ImGuiKey_COUNT];  // [LEGACY] Input: map of indices into the
                                 ...
                                 // Legacy backend will write into KeyMap[]
                                 // using ImGuiKey_ indices which are always
                                 // >512.
    bool
        KeysDown[ImGuiKey_COUNT];  // [LEGACY] Input: Keyboard keys that are
    ...
    void* ImeWindowHandle;  // = NULL   // [Obsoleted in 1.87] Set
                            // ImGuiViewport::PlatformHandleRaw instead. Set
                            // this to your HWND to get automatic IME cursor
                            // positioning.
#else
    void* _UnusedPadding;
#endif
    ...
    ImGuiKeyChord
        KeyMods;  // Key mods flags (any of
                  ...
                  // merged into flags. DOES NOT CONTAINS ImGuiMod_Shortcut
    ...
    ImS8 BackendUsingLegacyKeyArrays;  // -1: unknown, 0: using AddKeyEvent(),
                                       // 1: using legacy io.KeysDown[]
...
struct ImDrawList {
    // This is what you have to render
    ImVector<ImDrawCmd>
        CmdBuffer;  // Draw commands. Typically 1 command = 1 GPU
...
struct ImFontConfig {
    ...
    ImWchar EllipsisChar;   // -1       // Explicitly specify unicode codepoint
...
struct ImFontAtlas {
    ...
    int TexGlyphPadding;  // Padding between glyphs within texture in pixels.
                          ...
                          // (will also need to set AntiAliasedLinesUseTex =
                          // false).
...
struct ImFont {
    ...
    float Scale;  // 4     // in  // = 1.f      // Base font scale, multiplied
...
enum ImGuiViewportFlags_ {
    ...
    ImGuiViewportFlags_OwnedByApp =
        1 << 2,  // Platform Window: Was created/managed by the user
...
struct ImGuiPlatformIO {
    ...
    void (*Platform_SetWindowTitle)(
        ImGuiViewport* vp,
        const char* str);  // . . U . .  // Set platform window
