...
#define IM_BITARRAY_CLEARBIT(_ARRAY, _N) \
    ((_ARRAY[(_N) >> 5] &=               \
      ~((ImU32)1 << ((                   \
            _N)&31))))  // Macro version of ImBitArrayClearBit(): ensure
...
enum ImGuiItemFlags_ {
    ...
    // widgets)
    ImGuiItemFlags_ReadOnly = 1 << 7,  // false     // [ALPHA] Allow hovering
                                       // interactions but underlying
    ...
    ImGuiItemFlags_Inputable =
        1
        << 10,  // false     // [WIP] Auto-activate input mode when tab focused.
...
enum ImGuiButtonFlagsPrivate_ {
    ...
    ImGuiButtonFlags_NoNavFocus =
        1 << 18,  // don't override navigation focus when activated (FIXME: this
                  // is essentially used everytime an item uses
...
struct IMGUI_API ImGuiInputTextState {
    ...
    bool TextAIsValid;  // temporary UTF8 buffer is not initially valid before
                        // we make the widget active (until then we pull the
...
enum ImGuiInputFlags_ {
    ...
    ImGuiInputFlags_Repeat =
        1 << 0,  // Return true on successive repeats. Default
    ...
    ImGuiInputFlags_RouteFocused = 1 << 8,  // (Default) Register focused route:
                                            // Accept inputs if window is in
    ...
    ImGuiInputFlags_RouteGlobalLow =
        1 << 9,  // Register route globally (lowest priority: unless a focused
                 ...
                 // Global priority.
    ImGuiInputFlags_RouteGlobal = 1 << 10,  // Register route globally (medium
                                            // priority: unless an active item
    // registered the route, e.g. CTRL+A registered by InputText).
    ImGuiInputFlags_RouteGlobalHigh =
        1
        << 11,  // Register route globally (highest priority: unlikely you need
...
enum ImGuiDockNodeFlagsPrivate_ {
    ...
    ImGuiDockNodeFlags_NoTabBar = 1 << 12,  // Saved // Tab bar is completely
                                            // unavailable. No triangle in the
                                            // corner to enable it back.
    ImGuiDockNodeFlags_HiddenTabBar =
        1
        << 13,  // Saved // Tab bar is hidden, with a triangle in the corner to
...
enum ImGuiDebugLogFlags_ {
    ...
    ImGuiDebugLogFlags_EventMask_ =
        ...
        ImGuiDebugLogFlags_EventIO | ImGuiDebugLogFlags_EventDocking |
...
struct ImGuiContext {
    ...
    bool ActiveIdUsingAllKeyboardKeys;  // Active widget will want to read all
                                        ...
                                        // a shortcut for not taking ownership
                                        // of 100+ keys but perhaps best to not
                                        // have the inconsistency)
#ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
    ImU32
        ActiveIdUsingNavInputMask;  // If you used this. Since
                                    // (IMGUI_VERSION_NUM
    ...
    ImGuiID NavFocusScopeId;  // Identify a selection scope (selection code
                              // often wants to "clear other items" when landing
...
struct IMGUI_API ImGuiTable {
    ...
    ImGuiTableColumnIdx
        HoveredColumnBorder;  // Index of column whose right-border
...
IMGUI_API void MarkItemEdited(
    ImGuiID id);  // Mark data associated to given item as "edited",
...
IMGUI_API bool
IsItemToggledSelection();  // Was the last item selection toggled?
...
IMGUI_API void GetKeyChordName(ImGuiKeyChord key_chord,
                               char* out_buf,
                               int out_buf_size);
