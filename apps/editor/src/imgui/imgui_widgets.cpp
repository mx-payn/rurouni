...
static bool InputTextFilterCharacter(ImGuiContext* ctx,
    ...
    if (c < 0x20) {
        ...
        pass |= (c == '\n' &&
                 (flags &
                  ImGuiInputTextFlags_Multiline));  // Note that an Enter KEY
                                                    // will emit \r and be
                                                    // ignored (we poll for KEY
                                                    // in InputText() code)
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        const bool is_redo =
            (Shortcut(ImGuiMod_Shortcut | ImGuiKey_Y, id, f_repeat) ||
             (is_osx &&
              Shortcut(ImGuiMod_Shortcut | ImGuiMod_Shift | ImGuiKey_Z, id,
                       f_repeat))) &&
            !is_readonly && is_undoable;
...
bool ImGui::ColorEdit4(const char* label,
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB |
        ...
        const float w_item_one =
            ImMax(1.0f, IM_TRUNC((w_inputs - (style.ItemInnerSpacing.x) *
                                                 (components - 1)) /
                                 (float)components));
        const float w_item_last = ImMax(
            1.0f, IM_TRUNC(w_inputs - (w_item_one + style.ItemInnerSpacing.x) *
                                          (components - 1)));
    ...
    if (NavMoveRequestButNoResultYet() &&
        ...
        if (nav_earliest_child->ParentWindow == window &&
            ...
            g.NavDisableHighlight =
                true;  // Hide highlight for the current frame so
