...
/** units in pixel */
struct AtlasMetrics {
    TextureDistanceFieldType type;  // "msdf",
...
struct FontSpecification {
    ...
    UUID AtlasId;
    rr::graphics::AtlasMetrics AtlasMetrics;
    rr::graphics::FontMetrics FontMetrics;
    std::map<unicode_t, rr::graphics::GlyphMetrics> GlyphMetrics;
};

class Font : public Texture {
   public:
    Font(const FontSpecification& spec, std::weak_ptr<Texture> atlas);
    ...
    const GlyphMetrics& get_glyph_metrics(unicode_t codepoint);
    float get_kerning_distance(unicode_t left, unicode_t right);

   protected:
    rr::graphics::AtlasMetrics m_AtlasMetrics;
    rr::graphics::FontMetrics m_FontMetrics;
...
//             cereal::make_nvp("underlineThickness",
